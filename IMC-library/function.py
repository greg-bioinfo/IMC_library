...
def combine_marker(list_marker,path):
    ...
    colors=[2,0,1]
    pos=[20,60,100]
    colors_txt=[(0,0,255),(255,0,0),(0,255,0),(0,0,255)]
    name=""
    ...
    if os.path.isdir(path)==False:
        os.mkdir(path)
    for lame in os.listdir("./Lames_arcsinh"):
        print("** Lame: "+lame+" **")
        #if os.path.isdir(path+"/"+lame)==False:
            #os.mkdir(path+"/"+lame)
        for roi in os.listdir("./Lames_arcsinh/"+lame):
            print("    ROI: "+roi)
            #if os.path.isdir(path+"/"+lame+"/"+roi)==False:
                #os.mkdir(path+"/"+lame+"/"+roi)
            n=0
            for marker in list_marker:
                img_marker=plt.imread("./Lames_arcsinh/"+lame+"/"+roi+"/"+marker+".png")
                if n==0:
            ...
            for marker in list_marker:
                ...
                #img=arcsinh_std_thresh(img,thresh,cofactor,kernel)
            cv2.imwrite(path+"/"+roi+"_"+name+".png",normalize_255(img))
